...
1.루트노드가 언제나 최댓값 또는 최솟값을 가짐
-max heap, min heap
2.완전이진트리여야 함
이진탐색트리와의 차이점 : 느슨한 정렬이 되어있는 구조(같은 레벨의 원소들은 정렬되어 있지 않음)
->특정 키 값을 가지는 원소를 빠르게 검색할 수 없음
배열을 이용해서 구현(완전이진트리이기에 효과적)
왼쪽 자식의 번호 : 2*노드번호
오른쪽 자식의 번호 : 2*노드번호+1
부모노드 번호 : 노드번호//2
완전 이진 트리이므로 노드의 추가/삭제는 마지막 노드에서만
원소 삽입 복잡도 : 최악 O(logn) *최대 depth가 log_2 n이므로
원소 삭제 복잡도 : 최악 O(logn)
최대최소힙의 응용
우선순위큐 구현 시 양방향 연결  리스트를 이용한 구현보다 시간적 효율성을 갖는다.
정렬 시 시간적 효율성을 갖는다.O(nlogn)
